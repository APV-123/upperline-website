...
Run (Join-Path $PSScriptRoot 'property-intelligence-observations.integration.sql')
Run (Join-Path $PSScriptRoot 'property-intelligence-observations-adversarial.integration.sql')
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot 'run-property-intelligence-observations-concurrency.ps1') -DatabaseUrl $DatabaseUrl
if($LASTEXITCODE-ne 0){throw 'concurrency suite failed'}
Write-Host 'Property intelligence observation integration tests passed.'
