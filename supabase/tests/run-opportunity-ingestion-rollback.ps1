...
& $psql -X -v ON_ERROR_STOP=1 -d $DatabaseUrl -f (Join-Path $PSScriptRoot 'opportunity-ingestion-rollback.integration.sql')
if($LASTEXITCODE -ne 0){throw 'Rollback verification failed'}
Write-Host 'Opportunity ingestion rollback test passed.'
