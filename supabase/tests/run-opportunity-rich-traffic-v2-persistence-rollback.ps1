...
$files=@(
  ...
  (Join-Path $PSScriptRoot 'opportunity-rich-traffic-v2-persistence-rollback.integration.sql')
)
$arguments=@('-X','-v','ON_ERROR_STOP=1','-d',$DatabaseUrl)
foreach($file in $files){$arguments+=@('-f',$file)}
& $psql @arguments
if($LASTEXITCODE-ne 0){throw 'rich traffic V2 persistence rollback failed'}
Write-Host 'Rich traffic V2 persistence rollback tests passed.'
