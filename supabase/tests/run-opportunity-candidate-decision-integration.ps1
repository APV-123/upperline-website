...
Run (Join-Path $root 'supabase/migrations/20260824000100_create_candidate_decision_rpc.sql')
Run (Join-Path $PSScriptRoot 'opportunity-candidate-decision.integration.sql')
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot 'run-opportunity-candidate-decision-concurrency.ps1') -DatabaseUrl $DatabaseUrl
if($LASTEXITCODE-ne 0){throw 'candidate decision concurrency suite failed'}
Write-Host 'Opportunity candidate decision integration tests passed.'
