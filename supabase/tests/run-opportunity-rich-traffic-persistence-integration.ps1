...
$files=@(
  ...
  (Join-Path $PSScriptRoot 'opportunity-rich-traffic-persistence.integration.sql')
)
$arguments=@('-X','-v','ON_ERROR_STOP=1','-d',$DatabaseUrl)
foreach($file in $files){$arguments+=@('-f',$file)}
& $psql @arguments
if($LASTEXITCODE-ne 0){throw 'rich traffic persistence integration failed'}
Write-Host 'Rich traffic persistence integration tests passed.'
