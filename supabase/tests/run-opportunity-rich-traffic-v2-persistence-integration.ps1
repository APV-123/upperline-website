...
$files=@(
  ...
  (Join-Path $PSScriptRoot 'opportunity-rich-traffic-persistence-preflight.sql'),
  (Join-Path $root 'supabase/migrations/20260830000100_admit_rich_traffic_extraction_candidates.sql'),
  (Join-Path $root 'supabase/migrations/20260831000100_admit_rich_traffic_extraction_candidates_v2.sql'),
  (Join-Path $PSScriptRoot 'opportunity-rich-traffic-v2-persistence.integration.sql')
)
$arguments=@('-X','-v','ON_ERROR_STOP=1','-d',$DatabaseUrl)
foreach($file in $files){$arguments+=@('-f',$file)}
& $psql @arguments
if($LASTEXITCODE-ne 0){throw 'rich traffic V2 persistence integration failed'}
Write-Host 'Rich traffic V2 persistence integration tests passed.'
