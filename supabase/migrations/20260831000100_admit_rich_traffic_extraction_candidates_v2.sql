-- Phase 4C.6D.5: add the approved traffic_count/version-2 source-semantic
-- proposition without reinterpreting historical scalar or version-1 traffic.
...
declare v_cross_street jsonb; declare v_offset jsonb; declare v_subject_distance jsonb;
declare v_scalar_traffic boolean; declare v_rich_traffic boolean; declare v_rich_traffic_v2 boolean;
begin
  ...
  for v_candidate in select value from jsonb_array_elements(p_candidates) loop
    ...
    v_cross_street:=v_value->'crossStreet'; v_offset:=v_value->'crossStreetOffset';
    v_subject_distance:=v_value->'sourceRelativeSubjectDistance';
    v_rich_traffic_v2:=coalesce((
      v_path='traffic.vehiclesPerDay'
      and v_candidate->>'normalizedValueType'='json'
      and v_candidate->>'unit'='VEHICLES_PER_DAY'
      and v_candidate->>'groupKey'='traffic_count:2'
      and jsonb_typeof(v_value)='object'
      and v_value->>'kind'='traffic_count'
      and v_value->'schemaVersion'='2'::jsonb
      and jsonb_typeof(v_value->'count')='number'
      and v_value->>'count' ~ '^[1-9][0-9]*$'
      and v_value->>'unit'='vehicles_per_day'
      and v_value ?& array['kind','schemaVersion','count','unit','basis','sourceVolumeType','roadway','crossStreet','crossStreetOffset','sourceRelativeSubjectDistance','measurementTime']
      and not exists(select 1 from jsonb_object_keys(v_value) as k(key) where key not in
        ('kind','schemaVersion','count','unit','basis','sourceVolumeType','roadway','crossStreet','crossStreetOffset','sourceRelativeSubjectDistance','measurementTime'))
      and jsonb_typeof(v_basis)='object'
      and v_basis ?& array['normalized','sourceLiteral']
      and not exists(select 1 from jsonb_object_keys(v_basis) as k(key) where key not in ('normalized','sourceLiteral'))
      and v_basis->>'normalized' in ('VPD','ADT','AADT','unknown')
      and (jsonb_typeof(v_basis->'sourceLiteral')='string' or v_basis->'sourceLiteral'='null'::jsonb)
      and (jsonb_typeof(v_value->'sourceVolumeType')='string' or v_value->'sourceVolumeType'='null'::jsonb)
      and coalesce(v_basis->>'sourceLiteral','')<>'MPSI'
      and jsonb_typeof(v_roadway)='object'
      and v_roadway ? 'sourceLiteral'
      and not exists(select 1 from jsonb_object_keys(v_roadway) as k(key) where key<>'sourceLiteral')
      and jsonb_typeof(v_roadway->'sourceLiteral')='string'
      and (v_cross_street='null'::jsonb or (
        jsonb_typeof(v_cross_street)='object' and v_cross_street ? 'sourceLiteral'
        and not exists(select 1 from jsonb_object_keys(v_cross_street) as k(key) where key<>'sourceLiteral')
        and jsonb_typeof(v_cross_street->'sourceLiteral')='string'))
      and jsonb_typeof(v_offset)='object'
      and v_offset ?& array['distance','unit','direction']
      and not exists(select 1 from jsonb_object_keys(v_offset) as k(key) where key not in ('distance','unit','direction'))
      and ((v_offset->'distance'='null'::jsonb and v_offset->'unit'='null'::jsonb and v_offset->'direction'='null'::jsonb) or
        (jsonb_typeof(v_offset->'distance')='number' and (v_offset->>'distance')::numeric>=0 and v_offset->>'unit'='miles'
          and (v_offset->'direction'='null'::jsonb or v_offset->>'direction' in ('N','NE','E','SE','S','SW','W','NW'))))
      and jsonb_typeof(v_subject_distance)='object'
      and v_subject_distance ?& array['distance','unit']
      and not exists(select 1 from jsonb_object_keys(v_subject_distance) as k(key) where key not in ('distance','unit'))
      and ((v_subject_distance->'distance'='null'::jsonb and v_subject_distance->'unit'='null'::jsonb) or
        (jsonb_typeof(v_subject_distance->'distance')='number' and (v_subject_distance->>'distance')::numeric>=0 and v_subject_distance->>'unit'='miles'))
      and jsonb_typeof(v_time)='object'
      and v_time ?& array['role','precision','year','month','day']
      and not exists(select 1 from jsonb_object_keys(v_time) as k(key) where key not in ('role','precision','year','month','day'))
      and v_time->>'role'='measurement'
...
comment on function public.complete_opportunity_extraction_run(uuid,uuid,uuid,jsonb,jsonb) is
  'Atomically persists validated extraction output. Supports historical scalar traffic and exact traffic_count versions 1 and 2; all output remains untrusted pending human review.';
