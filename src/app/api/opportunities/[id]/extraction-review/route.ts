...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) { const { id } = await context.params; return authenticatedOpportunityEndpoint(async () => new SupabaseExtractionReviewRepository(createOpportunitySupabaseClient()).getLatestReview(id)); }
