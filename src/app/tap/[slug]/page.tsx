...
export default function TapPage({ params }: { params: { slug: string } }) {
  ...
  if (typeof window !== 'undefined' && (window as any).dataLayer) {
    (window as any).dataLayer.push({ event: 'tap_page_view', tap_owner: owner });
  }
  ...
  const wrap: React.CSSProperties = {
    background: BRAND.navy, color: '#fff', minHeight: '100dvh',
  ...
  const btn = (bg: string, color: string): React.CSSProperties => ({
    display: 'block', textAlign: 'center', padding: '16px 18px',
    borderRadius: 12, textDecoration: 'none', fontWeight: 600, marginTop: 12,
    background: bg, color, boxShadow: '0 6px 18px rgba(0,0,0,.15)',
  ...
  return (
    <div style={wrap}>
      <motion.div style={{ textAlign: 'center', marginBottom: 18 }}
        initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .35 }}>
        <img src="/Upperline-logo-inverted.png" alt="Upperline"
             style={{ width: 'min(160px, 40vw)', height: 'auto', margin: '0 auto 16px' }} />
      </motion.div>

      <motion.div style={card} initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .28 }}>
        <motion.img
          src={person.headshot} alt={person.name}
          ...
          initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .35 }}
        />

        <motion.h1 style={{ margin: '0 0 4px', fontSize: 22, letterSpacing: .3 }} {...fadeUp} transition={{ duration: .35 }}>
          {person.name}
        </motion.h1>

        <motion.p style={{ margin: '0 0 16px', fontSize: 14, color: BRAND.turq }} {...fadeUp} transition={{ duration: .35 }}>
          {person.title}
        </motion.p>

        <motion.a href={person.vcf} style={btn('#fff', '#000')} {...fadeUp} transition={{ duration: .35 }}
          onClick={() => (window as any).dataLayer?.push({ event: 'tap_vcard_click', tap_owner: owner })}>
          Save my contact (vCard)
        </motion.a>

        <motion.div {...fadeUp} transition={{ duration: .35 }}>
          <Link href={`/intake/contact?owner=${owner}`} style={btn(BRAND.navy === '#003a5d' ? '#31c8db' : '#31c8db', '#002333')}
            onClick={() => (window as any).dataLayer?.push({ event: 'tap_share_click', tap_owner: owner })}>
